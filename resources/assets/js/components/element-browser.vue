...
<md-dialog :md-active.sync="showDialog" ref="elementsdialog">
  ...
  <md-list>
    <md-list-item @click="selected = model[item.id]; title = item.title" v-for="item in model" :key="item.id">
    ...
    <md-list v-if="selected">
      ...
      <div v-for="item in selected.items" :key="item.id">
        <div v-if="item.id !== 'mergeexclusive' || item.id === 'mergeexclusive' && Object.keys(exclusive).length > 0">
          ...
          <a class="menu-item" @click="select(selected.id, item.id)" :id="item.id"><strong>{{item.title}}</strong></a>
          <p>
            {{item.description}}
          </p>
